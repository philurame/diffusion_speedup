...
class Trainer:
  ...
  def __init__(self, config):
    ...
    self.loss_fn = loss_registry[config.loss.name](device=config.device, coeffs=config.loss.coeffs)
  ...
  def train_epoch(self):
    ...
    for batch_start in range(0, train_size, batch_size):
      ...
      for mini_batch_start in range(0, _batch_size, mini_batch_size):
        ...
        rl_loss, loss_mean = self.rl_loss_fn(
          logprob,
      ...
      log_dict = grad_clip(self.config, self.rl_logits, self.timesteps_model, self.pipe)
