...
class Trainer:
  ...
  def __init__(self, config):
    ...
    max_timestep = self.pipe.max_timestep
    self.timesteps_model = TSModel(config.nfe, config.timesteps.param_method, config.timesteps.init_method,  max_timestep=max_timestep)
    ...
    self.rl_logits, self.optimizer = get_optimizer(config, self.timesteps_model, self.pipe)
    self.global_step = 1

    self.loss_fn = loss_registry[config.loss.name](device=config.device, coeffs=config.loss.get('coeffs'))
    self.rl_loss_fn = RLLoss(self.loss_fn) if config.rl.train else None

    if config.metric.period > 0:
      self.metric = metric_registry[config.metric.name](run_id = config.run_id)
  ...
  def train(self):
    ...
    for epoch in tqdm.tqdm(range(self.config.epochs)):
      ...
      if self.config.metric.period > 0 and epoch % self.config.metric.period == 0 and epoch:
        ...
        log_scalars(metric_results, self.global_step)
    ...
    for batch_start in range(0, train_size, batch_size):
      ...
      for mini_batch_start in range(0, _batch_size, mini_batch_size):
        ...
        if self.config.rl.train:
          gen_latents_samples, logprob = rl_sample(self.config, self.pipe, self.timesteps_model, self.rl_logits, **data)
          ...
          rl_loss, loss = self.rl_loss_fn(
            logprob,
            gen_latents_samples = gen_latents_samples,
            gen_imgs_samples    = gen_imgs_samples,
            **data
          )
          (rl_loss/_batch_size).backward()
        
        # GRADIENT BACKWARD LOGIC
        else:
      ...
      log_dict = grad_clip(self.config, self.timesteps_model, self.pipe, rl_logits=self.rl_logits)
