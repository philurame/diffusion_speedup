...
def train(config):
  ...
  pipe.scheduler.lin_coeffs = [
    [2.6113338470458984, 0, 0, -1.6114189624786377],
    [2.3594629764556885, 0, 0.5224075317382812, -1.8823120594024658],
    [2.132323741912842, 0, 0.4387214481830597, -1.5729000568389893],
    [1.9274438619613647, 0, 0.3639830946922302, -1.2977262735366821],
    [1.7338180541992188, 0, 0.2945857346057892, -1.0455070734024048],
    [1.577396273612976, 0, 0.2548947036266327, -0.8716773986816406],
    [1.4267008304595947, 0, 0.21973039209842682, -0.7224735617637634],
    [1.290662169456482, 0, 0.206050843000412, -0.6245517730712891],
    [1.1678284406661987, 0, 0.21967783570289612, -0.5804626941680908],
    [1.0568965673446655, 0, 0, -0.3322421908378601],
  ]
  pipe.scheduler.lin_coeffs = torch.tensor(pipe.scheduler.lin_coeffs, dtype=torch.float32, requires_grad=True)
  ...
  optimizer = optim.Adam([timesteps_logits, pipe.scheduler.gammas, pipe.scheduler.powers], lr=config.learning_rate)
  # ===============================================================================
  ...
  for epoch in range(config.epochs):
    ...
    for i in tqdm.tqdm(range(0, config.train_size, config.batch_size)):
      ...
      # log train stats
      wandb.log({
        'ts_grad_norm': timesteps_logits.grad.norm(2).item(),
        'lin_grad_norm': pipe.scheduler.lin_coeffs.grad.norm(2).item(),
...
def validate(config, global_step, pipe, ts_param, timesteps_logits, ddim_features, ddim_imgs, lpips_model):
  ...
  wandb_log_ts(timesteps, global_step=global_step, key="timesteps", xlabel="NFE", ylabel="TS")
  wandb_log_ts(pipe.scheduler.lin_coeffs, global_step=global_step, key="lin_coeffs", xlabel="NFE", ylabel="lin_coeff")
...
def main(**kwargs):
  ...
  wandb.init(project="EDM_TRAIN_TS_EPS", config={
