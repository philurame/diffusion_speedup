...
class COEFUP41(SETDEIS2):
  ...
  def step(self, model_output, sample=None, **kwargs):
    ...
    if self.multiply_operator == "@":
      identity = torch.eye(128, device=A.device, dtype=model_output.dtype).unsqueeze(0).expand(4, -1, -1)
    elif self.multiply_operator == "*":
      identity = torch.ones(4,128,128, device=A.device, dtype=model_output.dtype)
    
    # upsample deltas to 4 x 128 x 128:
    deltas = [(identity+self.upsample(d)).to(model_output.device) for d in deltas][:self.step_index+2]
