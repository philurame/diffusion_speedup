...
def train(config):
  ...
  pipe.scheduler.gammas = torch.tensor([1.]*10, dtype=torch.float32, requires_grad=True)
  pipe.scheduler.powers = torch.tensor([1.]*10, dtype=torch.float32, requires_grad=True)
  ...
  for epoch in range(config.epochs):
    ...
    for i in tqdm.tqdm(range(0, config.train_size, config.batch_size)):
      ...
      wandb.log({
        ...
        'gamma_grad_norm': pipe.scheduler.gammas.grad.norm(2).item(),
        'power_grad_norm': pipe.scheduler.powers.grad.norm(2).item(),
...
def validate(config, global_step, pipe, ts_param, timesteps_logits, ddim_features, ddim_imgs, lpips_model):
  ...
  wandb_log_ts(pipe.scheduler.gammas, global_step=global_step, key="gammas", xlabel="NFE", ylabel="gamma")
  wandb_log_ts(pipe.scheduler.powers, global_step=global_step, key="powers", xlabel="NFE", ylabel="power")
