...
class Discriminator(nn.Module):
  ...
  def forward(self, latent, timesteps, encoder_hidden_states, added_cond_kwargs: Optional[Dict[str, torch.Tensor]] = None):
    # take cond part from (uncond, cond) input
    if encoder_hidden_states.shape[0] > latent.shape[0]:
      encoder_hidden_states = encoder_hidden_states.chunk(2)[-1]
      # latent = torch.cat([latent] * 2)

    feat_list = self.unet.forward(self.unet, latent, timesteps, encoder_hidden_states, added_cond_kwargs=added_cond_kwargs)
