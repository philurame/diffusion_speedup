...
def run_task(task, ROOT, wandb_key, metrics, device_queue, save_gen, num_samples, batch_size):
  if len(task) == 5:
...
if __name__ == '__main__':
  ...
  parser.add_argument("--wandb_key", type=str, default='_', help="WandB API key")
  parser.add_argument("--num_samples", type=int, default=30_000)
  parser.add_argument("--batch_size", type=int, default=16)
  parser.add_argument("--save_gen", type=int, default=0)
  ...
  with ThreadPoolExecutor(max_workers=max_workers) as executor:
    ...
    for task in all_tasks:
      futures.append(executor.submit(run_task, task, ROOT, wandb_key, metrics, device_queue, save_gen, num_samples, batch_size))
