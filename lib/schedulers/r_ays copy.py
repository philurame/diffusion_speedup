...
class AYSScheduler(SchedulerMixin):
  ...
  def sigmas_to_timesteps_flow_matching(self, sigmas):
    timesteps = (sigmas[:-1] * 1000)
    return torch.tensor(timesteps).to(torch.float32)
  
  def timesteps_to_sigmas_flow_matching(self, timesteps):
    sigmas = torch.cat([timesteps / 1000., 0.])
