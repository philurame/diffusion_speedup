...
def main(**kwargs):
  ...
  proj_name = config['model'].replace('BASE', 'TRAIN')
  run_name = f"{config['optimizer']}_nfe:{config['nfe']}_L:{config['loss']}_T:{config['solver']['teacher']}_I:{config['solver']['init']}"
  ...
  if config['rl']['train']:
    run_name += f"_rl:[{config['rl']['num_samples']}]"
  
  lrs = [config[i]['lr'] for i in ['timesteps', 'unet_timesteps', 'solver', 'adv', 'rl']]
  ...
  wandb.init(project=proj_name, config=config, name=run_name, dir="/workspace-SR008.fs2/philurame/wandb", settings={"_service_wait": 600,"init_timeout": 600})
