...
def main(config, device):
    ...
    train_dataloader = DataLoader(coco.anns[:args.max_samples], batch_size=args.train_batch_size, shuffle=False) # ПОПРАВИТЬ
    # train_dataloader = DataLoader(coco.anns[:3], batch_size=args.train_batch_size, shuffle=False) # ПОПРАВИТЬ
    # train_dataloader = DataLoader([coco.anns[2]], batch_size=1, shuffle=False)
    # print(f"\n\nWORK WITH ONLY PROMPT:\n{coco.anns[2]}\n\n")
    val_dataloader = DataLoader(coco.anns[-args.max_samples:], batch_size=args.val_batch_size, shuffle=False)
